...
public:
  ...
  void Init()
  {
    ...
    m_scoreTxt = new TTFText(ABT::FontPath("techno_hideo.ttf"), 45);
    ...
    m_gameOverTxt = new TTFText(ABT::FontPath("techno_hideo.ttf"), 45);
    ...
    m_quitReplay = new TTFText(ABT::FontPath("techno_hideo.ttf"), 45);
    ...
    m_crosshair = new ABT::Surface(ABT::TexturePath("crosshair.png"), glm::vec2(0.5f, 0.5f));
    m_crosshair->Load();

    m_sscp = glm::ivec2(m_windowWidth / 2, m_windowHeight / 2);
    m_crosshair->m_node->Translate(glm::vec3(m_sscp.x, m_sscp.y, 0));

    m_backGround.m_mesh = ABT::Main::GetInstance()->m_meshMan.Create(ABT::MeshPath("earthBg.mesh"));
    m_paralaxLayer.m_mesh = ABT::Main::GetInstance()->m_meshMan.Create(ABT::MeshPath("starParalaxLayer.mesh"));
    m_paralaxLayer.m_mesh->m_material->m_renderState.blendFunction = ABT::BlendFunction::SRC_ALPHA_ONE_MINUS_SRC_ALPHA;
    m_paralaxLayer.m_mesh->m_material->m_renderState.depthTestEnabled = false;

    m_lazerShotWav = ABT::Main::GetInstance()->m_audioMan.Create(ABT::AudioPath("lazerShot.wav"));
    m_lazerShotSource.AttachAudio(m_lazerShotWav);
    m_lazerShotSource.SetVolume(0.5f);

    m_explosionWav = ABT::Main::GetInstance()->m_audioMan.Create(ABT::AudioPath("explosion.wav"));
    m_explosionSource.AttachAudio(m_explosionWav);
    m_explosionSource.SetVolume(0.1f);

    m_shipExplosionWav = ABT::Main::GetInstance()->m_audioMan.Create(ABT::AudioPath("shipExplosion.wav"));
    m_shipExplosionSource.AttachAudio(m_shipExplosionWav);

    m_ambientLoopWav = ABT::Main::GetInstance()->m_audioMan.Create(ABT::AudioPath("ambientLoop.wav"));
    m_ambientLoopSource.AttachAudio(m_ambientLoopWav);
    m_ambientLoopSource.SetLoop(true);
    ABT::AudioPlayer::Play(&m_ambientLoopSource);
  }
  ...
  void MoveShip()
  {
    ...
    m_spaceShip->m_node->m_orientation = glm::angleAxis(glm::radians(moveVec.x * 50), ABT::Z_AXIS);
  }
  ...
  void CheckProjectileMeteorCollision()
  {
    for (int i = (int)m_meteorManager.m_meteors.size() - 1; i > -1; i--)
    {
      ...
      if (markForDel)
      {
        ABT::Node* meteorNode = m_meteorManager.m_meteors[i]->m_node;
        ...
        ABT::AudioPlayer::Play(&m_explosionSource);
      }
    }
  }
  ...
  void CheckShipMeteorCollision()
  {
    for (auto meteor : m_meteorManager.m_meteors)
    {
      if (m_spaceShip->CheckShipSphereCollision(meteor->m_node->m_translation, meteor->m_collisionRadius))
      {
        ...
        ABT::AudioPlayer::Play(&m_shipExplosionSource);
      }
    }
  }
  ...
  ABT::Camera m_cam;
  ABT::Renderer m_renderer;
  Ship* m_spaceShip = nullptr;
  ABT::Surface* m_crosshair = nullptr;
  glm::ivec2 m_sscp;
  ...
  ABT::Drawable m_backGround;
  ABT::Drawable m_paralaxLayer;
  TTFText* m_scoreTxt;
  TTFText* m_gameOverTxt;
  TTFText* m_quitReplay;
  int m_score = 0;
  bool m_restartSignaled = false;
  std::shared_ptr<ABT::Audio> m_lazerShotWav;
  std::shared_ptr<ABT::Audio> m_explosionWav;
  std::shared_ptr<ABT::Audio> m_shipExplosionWav;
  std::shared_ptr<ABT::Audio> m_ambientLoopWav;
  ABT::AudioSource m_lazerShotSource;
  ABT::AudioSource m_explosionSource;
  ABT::AudioSource m_shipExplosionSource;
  ABT::AudioSource m_ambientLoopSource;

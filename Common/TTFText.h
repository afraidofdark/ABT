...
private:
  void CreateTexture()
  {
    ...
    std::shared_ptr<ABT::Texture> texPtr(new ABT::Texture());
    texPtr->m_textureId = textureId;
    texPtr->m_width = m_sdlSurface->w;
    texPtr->m_height = m_sdlSurface->h;

    ABT::Node* backUp = nullptr;
    if (m_surface != nullptr)
      backUp = new ABT::Node(*m_surface->m_node);

    SafeDel(m_surface);
    m_surface = new ABT::Surface(texPtr, glm::vec2());

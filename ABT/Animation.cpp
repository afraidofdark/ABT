...
ABT::Animation::Animation()
{
}

ABT::Animation::Animation(std::string file)
{
  m_file = file;
}
...
void ABT::Animation::GetCurrentPose(Node* node)
{
  if (m_keys.empty())
    return;
...
void ABT::Animation::GetCurrentPose(Skeleton* skeleton)
{
  if (m_keys.empty())
    return;
...
float ABT::Animation::GetDuration()
{
  return m_duration;
}

void ABT::Animation::Load()
{
...
void ABT::Animation::Init(bool flushClientSideArray)
{
}

ABT::Animation* ABT::Animation::GetCopy()
{
  return new Animation(*this);
}

void ABT::Animation::GetNearestKeys(const std::vector<Key>& keys, int& key1, int& key2, float& ratio)
{
  // Find nearset keys.
  key1 = -1;
  key2 = -1;
...
void ABT::AnimationPlayer::AddRecord(Entity* entity, Animation* anim)
{
  m_records.push_back(std::pair<Entity*, Animation*>(entity, anim));
}

void ABT::AnimationPlayer::RemoveRecord(Entity* entity, Animation* anim)
{
  int index = 0;
...
void ABT::AnimationPlayer::Update(float deltaTimeSec)
{
  int index = 0;
